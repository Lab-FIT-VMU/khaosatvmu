...
} from '../components/graduation2/GraduationFilterMultiSelect';
import {
  GraduationImportDialog,
  type GraduationImportTarget,
} from '../components/graduation2/GraduationImportDialog';
...
const reviewDateLabel = (reviewMonth: number | null, reviewYear: number | null) =>
  reviewMonth !== null && reviewYear !== null
    ...
    : 'Chưa có tháng/năm xét';

const periodOptionLabel = (period: GraduationManagedPeriod) =>
  `Đợt ${period.roundNumber} · ${reviewDateLabel(period.reviewMonth, period.reviewYear)}`;
...
export function GraduationAnalytics2Page({ view }: { view: GraduationAnalyticsView }) {
  ...
  // Chỉ liệt kê năm học đã có khoá học, không sinh dải năm theo đồng hồ hệ thống.
  ...
  const loadPeriods = useCallback(async (preferredPeriodId?: number) => {
    ...
    setError(null);
    try {
      const next = await graduationAnalytics2Api.managedPeriods();
      setPeriods(next);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const years = await graduationAnalytics2Api.academicYearStarts();
        if (cancelled) return;
        setAcademicYears(years);
        setAcademicYearStart((current) =>
          years.includes(current) ? current : years[0] ?? current);
      } catch {
        if (!cancelled) setError('Không tải được danh sách năm học.');
      }
    })();
  ...
  useEffect(() => {
    ...
    graduationAnalytics2Api.exploreV3({
      mode,
  ...
  const handlePeriodDeleted = async (period: GraduationManagedPeriod) => {
    ...
    await loadPeriods();
    toast.success(`Đã xóa toàn bộ dữ liệu đã import của Đợt ${period.roundNumber}.`);
  };
  ...
  const toggleHistory = async (periodId: number) => {
    ...
    try {
      setRevisions(await graduationAnalytics2Api.revisions(periodId));
    } catch {
        ...
        <table>
          ...
          <tbody>{rounds.map((round) => {
            ...
            return <tr key={round}>
              ...
              <td>{period ? reviewDateLabel(period.reviewMonth, period.reviewYear) : <span className="graduation-muted">Chọn khi tải lên</span>}</td>
