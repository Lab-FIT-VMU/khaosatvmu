...
public static class GraduationExploreCalculator
{
    ...
    /// <summary>
    /// Chương trình chuẩn 4 năm. Khoá 62 nhập năm học 2021-2022 thì đúng hạn là
    /// tốt nghiệp trong năm học 2024-2025, tức năm học của đợt hơn năm học nhập
    /// học đúng ba bậc.
    /// </summary>
    private const int StandardProgramYears = 4;

    /// <summary>
    /// Đúng hạn xét theo khoá học chứ không theo hệ đào tạo: sinh viên tốt nghiệp
    /// trong năm học thứ <see cref="StandardProgramYears"/> kể từ khi nhập học.
    /// Khoá không đọc được số thì không tính là đúng hạn.
    /// </summary>
    private static bool IsOnTime(
        GraduationExploreCell cell,
        IReadOnlyDictionary<long, GraduationExplorePeriod> periodById)
    {
        if (!periodById.TryGetValue(cell.PeriodId, out var period))
        {
            return false;
        }
        if (!TryGetCohortAcademicYearStart(cell.CohortCode, out var cohortYearStart))
        {
            return false;
        }
        return period.AcademicYearStart - cohortYearStart == StandardProgramYears - 1;
    }

    /// <summary>Mã khoá có thể là '62' hoặc 'K62'; lấy phần số để suy năm nhập học.</summary>
    private static bool TryGetCohortAcademicYearStart(string cohortCode, out int academicYearStart)
    {
        ...
        {
            academicYearStart = BaseAcademicYearStart + (number - BaseCohortNumber);
            return true;
        }
        academicYearStart = 0;
        return false;
